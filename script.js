...
const purchaseOrRefund = (game) => {
  const purchaseButton = document.createElement('button');
  purchaseButton.textContent = 'Purchase';
  purchaseButton.addEventListener('click', () => {
    alert(`Game ${game.name} has been purchased.`);
  });

  const refundButton = document.createElement('button');
  refundButton.textContent = 'Refund';
  refundButton.addEventListener('click', () => {
    alert(`Game ${game.name} has been refunded.`);
  });

  const gameButtons = document.createElement('div');
  gameButtons.appendChild(purchaseButton);
  gameButtons.appendChild(refundButton);

  gameInfo.innerHTML = '';
  gameInfo.appendChild(gameButtons);
};
